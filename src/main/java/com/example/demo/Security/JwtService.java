...
public class JwtService {
    ...
    public JwtService(RefreshRepository refreshRepository, SecretKey key) {
    ...
    public Date extractByExpirationDate(String token){
        return Jwts.parser()
                .verifyWith(key)
                .build()
                .parseSignedClaims(token)
    ...
    public boolean isExpired(String token){
        if(extractByExpirationDate(token).before(new Date())){
            return true;
        }return false;
    }
